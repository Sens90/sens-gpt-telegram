...
def choose_scene(player,category='random',custom_environment=None):
    ...
    if custom: place=custom[:300]; source='Ambientazione richiesta dall utente'
...
def build_visual_prompt(player,category='random',custom_environment=None):
    ...
    environment_lock=(f"AMBIENTAZIONE LOCK ASSOLUTO: l'utente ha richiesto ESATTAMENTE «{scene['custom_environment']}». Riproduci questa ambientazione e nessun'altra. NON sostituirla, NON reinterpretarla come un luogo casuale, NON scegliere Starr Park o altri mondi Brawl Stars. Se stile e luogo sono entrambi richiesti, applica lo stile AL luogo richiesto. " if scene.get('custom_environment') else "AMBIENTAZIONE LIBERA: solo in assenza di una richiesta esplicita puoi usare la scena proposta. ")
    creative=(player.get('ai_creative_style') or '').strip()
    creative_lock=(f"RICHIESTA CREATIVA UTENTE: applica al Brawler il tema «{creative}» mantenendo riconoscibili identita, silhouette, volto e caratteristiche fondamentali della reference; non presentarlo come una skin ufficiale esistente. " if creative else "")
    return (style+environment_lock+creative_lock+f"Ambientazione OBBLIGATORIA: {scene['place']}. Idea narrativa: {scene['concept']}. Profilo: {name} {tag}. Personaggio: {identity}. REGOLA BRAWLER: stesso personaggio della reference, non reinterpretarlo. REGOLA TESTI: NON generare alcun testo, numero, statistica, logo, stemma, watermark, badge, box, card, pannello UI o HUD. Questi elementi verranno aggiunti dopo in modo deterministico. Mantieni il Brawler protagonista e lascia zone visivamente libere per il compositing finale."),scene
