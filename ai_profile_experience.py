...
def build_visual_prompt(player,category='random',custom_environment=None):
    ...
    return (style+f"Ambientazione OBBLIGATORIA: {scene['place']}. Idea narrativa: {scene['concept']}. Profilo: {name} {tag}. Personaggio: {identity}. REGOLA BRAWLER: stesso personaggio della reference, non reinterpretarlo. REGOLA TESTI: NON generare alcun testo, numero, statistica, logo, stemma, watermark, badge, box, card, pannello UI o HUD. Questi elementi verranno aggiunti dopo in modo deterministico. Mantieni il Brawler protagonista e lascia zone visivamente libere per il compositing finale."),scene
