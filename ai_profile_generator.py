...
def generate_scene(player, category="random"):
    ...
    content = [
        {"type": "text", "text": prompt + " Usa l'immagine allegata come reference VISIVA OBBLIGATORIA del personaggio. Non copiare testo o numeri presenti nella reference."},
    ...
    for item in message.get("images") or []:
        ...
        if url.startswith("http"):
            image = requests.get(url, timeout=30); image.raise_for_status(); return image.content, scene
...
def overlay_stats(image_bytes, player, logo_path="assets/titani_logo.jpg"):
    ...
    left = (image.width - target_w) // 2; top = (image.height - target_h) // 2
    ...
    for l1,v1,l2,v2 in rows:
        ...
        draw.text((560,y),l2,font=_font(22),fill=(190,195,210,255)); draw.text((560,y+28),v2,font=_font(32,True),fill="white")
        y += 82
    try:
        logo=Image.open(logo_path).convert("RGBA"); logo.thumbnail((150,150), Image.Resampling.LANCZOS)
        overlay.alpha_composite(logo,(850,65))
    except Exception: pass
    image = Image.alpha_composite(image.convert("RGBA"), overlay).convert("RGB")
    out=io.BytesIO(); image.save(out,"JPEG",quality=94,optimize=True); out.seek(0); return out
