...
def _same_month(value):
    if not value: return False
    return str(value)[:7] == _month_key()


def list_premium_users(supabase_url, service_key):
    if not supabase_url or not service_key: raise RuntimeError("Supabase non configurato")
    r=requests.get(f"{supabase_url}/rest/v1/ai_profile_privileged_users",headers=_headers(service_key),params={"select":"telegram_user_id,note,created_at","order":"created_at.asc"},timeout=15); r.raise_for_status(); return r.json()
...
def add_premium_user(user_id,note,supabase_url,service_key):
    ...
    r=requests.post(f"{supabase_url}/rest/v1/ai_profile_privileged_users",headers={**_headers(service_key),"Prefer":"return=minimal"},json={"telegram_user_id":user_id,"note":(note or "Premium AI")[:200]},timeout=15); r.raise_for_status(); return "added"


def remove_premium_user(user_id,supabase_url,service_key):
    r=requests.delete(f"{supabase_url}/rest/v1/ai_profile_privileged_users",headers={**_headers(service_key),"Prefer":"return=minimal"},params={"telegram_user_id":f"eq.{int(user_id)}"},timeout=15); r.raise_for_status(); return True


def get_monthly_extra(user_id,supabase_url,service_key):
    r=requests.get(f"{supabase_url}/rest/v1/ai_profile_quota_bonus",headers=_headers(service_key),params={"telegram_user_id":f"eq.{int(user_id)}","select":"permanent_extra,updated_at","limit":"1"},timeout=15); r.raise_for_status(); rows=r.json()
    if not rows or not _same_month(rows[0].get("updated_at")):return 0
    return int(rows[0].get("permanent_extra") or 0)


def change_monthly_extra(user_id,delta,supabase_url,service_key):
    current=get_monthly_extra(user_id,supabase_url,service_key); new_value=max(0,current+int(delta))
    r=requests.post(f"{supabase_url}/rest/v1/ai_profile_quota_bonus",params={"on_conflict":"telegram_user_id"},headers={**_headers(service_key),"Prefer":"resolution=merge-duplicates,return=minimal"},json={"telegram_user_id":int(user_id),"permanent_extra":new_value,"updated_at":datetime.now(timezone.utc).isoformat()},timeout=15); r.raise_for_status(); return current,new_value


def _is_owner(message):return bool(message.from_user and int(message.from_user.id)==OWNER_TELEGRAM_ID)
def _can_manage_generations(message):return bool(message.from_user and int(message.from_user.id) in GENERATION_MANAGERS)
...
async def handle_premium_command(message,context,question,supabase_url,service_key):
    ...
    if generation_delta is not None:
        ...
        try:
            ...
            if generation_delta<0 and old==0:await message.reply_text(f"{display} non ha generazioni extra da rimuovere.")
            else:
                requested=abs(generation_delta)
                if generation_delta<0 and abs(actual_delta)<requested:detail=f"Rimosse {abs(actual_delta)} generazioni extra (non era possibile scendere sotto 0)."
                else:detail=f"Generazioni extra {'aggiunte' if actual_delta>0 else 'rimosse'}: {abs(actual_delta)}."
                await message.reply_text(f"{detail}\nUtente: {display}.\nExtra di questo mese: {new}.\nQuota del mese: {base+new} generazioni.\nIl mese prossimo tornerà automaticamente alla quota base di {base}.")
        ...
        return True
    if not _is_owner(message):await message.reply_text("Questo comando è riservato esclusivamente a Sens."); return True
    if q in {"lista premium","premium list"}:
        try:
            rows=list_premium_users(supabase_url,service_key); lines=["UTENTI PREMIUM AI"] if rows else ["Nessun utente Premium AI configurato."]
            for row in rows:lines.append(f"- {row.get('note') or 'Utente'} — ID {row['telegram_user_id']} — 8 generazioni/mese + eventuali extra del mese")
            lines.append(f"Posti disponibili: {MAX_PREMIUM_USERS-len(rows)}/{MAX_PREMIUM_USERS}"); await message.reply_text("\n".join(lines))
        except Exception as exc:
    ...
    if q in {"rendimi premium","rimuovimi premium"}:target=message.from_user
        ...
        target_message=message.reply_to_message; target=target_message.from_user if target_message else None
        if not target or target.is_bot:await message.reply_text("Rispondi a un messaggio della persona che vuoi aggiungere o rimuovere e usa questo comando."); return True
    display=target.full_name or target.username or str(target.id)
    try:
        if q in {"rendi premium","rendimi premium"}:
            ...
            else:await message.reply_text(f"{display} è ora un utente Premium AI.\nQuota: 8 generazioni AI al mese + eventuali extra del mese.")
        else:remove_premium_user(target.id,supabase_url,service_key); await message.reply_text(f"Premium AI rimosso da {display}.")
    except Exception as exc:
