...
def fetch_brawltrack_player(player_tag):
    ...
    try:
        ...
        if club_tag:
            clean_club_tag=_clean_tag(club_tag); club_tag="#"+clean_club_tag if clean_club_tag else str(club_tag).strip()
...
def _live_club(player_tag,timeout=15):
    ...
    try:
        r=requests.get(f"https://brawlify.com/it/player/{tag}?refresh={int(time.time())}",headers={"User-Agent":"Mozilla/5.0 (SensGPT-TitaniAbusivi/1.0)","Cache-Control":"no-cache","Pragma":"no-cache","Accept-Language":"it-IT,it;q=0.9,en;q=0.8"},timeout=timeout)
        ...
        raw_tag=href.group(1).upper(); club_tag="#"+raw_tag
        anchor=re.search(r'<a[^>]+href=["\']/(?:it/)?club/(?:%23|#)?'+re.escape(raw_tag)+r'[^"\']*["\'][^>]*>(.*?)</a>',page,re.I|re.S)
        if anchor:
        ...
        cr=requests.get(f"https://brawlify.com/it/club/{raw_tag}?refresh={int(time.time())}",headers={"User-Agent":"Mozilla/5.0 (SensGPT-TitaniAbusivi/1.0)","Cache-Control":"no-cache"},timeout=timeout)
        if cr.status_code==200:
            cp=html.unescape(cr.text)
            title=re.search(r"<title>\s*(.*?)\s*(?:#|—|-).*?</title>",cp,re.I|re.S)
            if title:
                name=re.sub(r"\s+"," ",re.sub(r"<[^>]+>"," ",title.group(1))).strip()
                if name:return name,club_tag
        return None,club_tag
    except Exception as e:
        print("PROFILE CLUB LIVE failure:",repr(e),flush=True); return None,None

# sitecustomize is guaranteed to load on Render; usercustomize is not.
...
def _parse_history_dt(row):
    try:
        dt=datetime.fromisoformat(str(row.get("recorded_at") or "").replace("Z","+00:00"))
        return dt if dt.tzinfo else dt.replace(tzinfo=timezone.utc)
    except Exception:return None

def _rome_changes(history,current):
    now_utc=datetime.now(timezone.utc); now_local=now_utc.astimezone(ROME)
    start_today=now_local.replace(hour=0,minute=0,second=0,microsecond=0).astimezone(timezone.utc)
    parsed=[]
    ...
    for days,key in ((7,"7d"),(15,"15d"),(30,"30d"),(90,"90d")):
        old=at_or_before(now_utc-timedelta(days=days)); changes[key]=int(current)-old if old is not None else None
    return changes
...
def _same_day_changes(base_calculator,history,current):
    ...
    fixed=_rome_changes(history,current); changes["today"]=fixed.get("today"); return changes
...
def _patch_app_runtime():
    for _ in range(2400):
        ...
        if main and hasattr(main,"calculate_trophy_changes") and hasattr(main,"community"):
            ...
            print("PROFILE TROPHY ROME SITECUSTOMIZE INSTALLATA",flush=True)
            return
        time.sleep(0.25)
