...
BrawlTrack is the primary meta/map/comp source in app.py. This module keeps
Brawl Planet only for verified live rotation and as a fallback when BrawlTrack
has no usable Ladder or Classificata datum.
"""
import csv
import gzip
import io
...
def safe_get(path, ttl=300):
    try: return get_json(path, 60 if path == "event_rotation.json.gz" else ttl)
        ...
        LOG.warning("LIVE_MAPS source unavailable path=%s error=%s", path, type(error).__name__); return None
...
def _install_direct_meta_route():
    try:
        ...
        print("META IMPORT ROUTE ERRORE:",repr(exc),flush=True); return
        ...
        if re.fullmatch(r"(?:il\s+)?meta(?:\s+attuale|\s+di\s+adesso|\s+ora)?[?!.]*",q):
            try:
                ...
                for query in queries:
                    response=requests.post("https://api.tavily.com/search",json={"api_key":key,"query":query,"search_depth":"advanced","max_results":12,"include_raw_content":True,"include_answer":False,"include_domains":["brawltrack.app"]},timeout=20)
